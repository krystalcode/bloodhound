...
--   primary key for the document, if provided together with the PUT method.
--   The POST method should be given with an empty DocId, so that the ID will
--   automatically generated by ElasticSearch.
--
-- >>> resp <- runBH' $ indexDocument' "put" testIndex testMapping defaultIndexDocumentSettings exampleTweet (DocId "1")
-- >>> print resp
...
indexDocument' :: (ToJSON doc, MonadBH m) => String -> IndexName -> MappingName
                 -> IndexDocumentSettings -> doc -> DocId -> m Reply
indexDocument' method (IndexName indexName)
  (MappingName mappingName) cfg document (DocId docId)
  | method == "put" = bindM2 put url (return body)
  | method == "post" = bindM2 post url (return body)
...
-- | 'indexDocument' provides a way to save a new or update an existing document
-- to ElasticSearch. The document ID must be provided.
indexDocument :: (ToJSON doc, MonadBH m) => IndexName -> MappingName
                 -> IndexDocumentSettings -> doc -> DocId -> m Reply
indexDocument (IndexName indexName)
  (MappingName mappingName) cfg document (DocId docId) =
  indexDocument' "put" (IndexName indexName) (MappingName mappingName) cfg document (DocId docId)

-- | 'indexDocument' provides a way to save a new or update an existing document
-- to ElasticSearch. The document ID will be automatically generated by
-- ElasticSearch.
indexDocumentAutoID :: (ToJSON doc, MonadBH m) => IndexName -> MappingName
                 -> IndexDocumentSettings -> doc -> m Reply
indexDocumentAutoID (IndexName indexName)
  (MappingName mappingName) cfg document =
  indexDocument' "post" (IndexName indexName) (MappingName mappingName) cfg document (DocId "")
